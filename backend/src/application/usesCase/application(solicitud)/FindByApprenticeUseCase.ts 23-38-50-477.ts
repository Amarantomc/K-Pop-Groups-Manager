...
export class FindByApprenticeUseCase {
    ...
    async execute(apprenticeId:number,applicationId: number): Promise<Application[]> {
        if (!applicationId) {
            throw new Error('Application ID must be provided');
        }

        const applications = await this.applicationRepository.findByApprentice(apprenticeId,applicationId);
        if (!applications) {
